...
- [x] **Q-C1 — `UnityW` liveness guards are not GC-safe → use-after-free on every async completion path.**
...
- [x] **Q-I1 — TLS peer verification disabled while sending the BeatLeader login cookie.**
...
- [x] **Q-I2 — Unvalidated `hash` builds the download URL and extraction dir; vendored zip honors symlink entries.**
...
- [x] **Q-I3 — Failed/partial extraction leaves a corrupt level folder that SongCore then scans.**
...
- [x] **Q-I4 — "0 follows", "no scores", "expired cookie", and "network down" all collapse into a misleading login error.**
...
- [x] **Q-I5 — No `curl_global_init`, no `CURLOPT_NOSIGNAL`.**
...
- [x] **Q-I6 — Sprite cache unbounded; `ClearCache()` is dead code.**
...
- [x] **Q-I7 — Refresh completion destroys the filter dropdown even while it's open → possible stuck input blocker.**
...
- [x] **Q-M1** — Modal not hidden on the refresh-failure path (src/FeedView.cpp:386-397; success path hides at :422).
- [x] **Q-M2** — TMP rich-text injection: network strings concatenated into markup unescaped (src/FeedCell.cpp:78-79, include/Format.hpp:49-80, src/FeedView.cpp:172-183). Map titles like `<3` break rendering. One escaping helper.
- [x] **Q-M3** — Detached workers have no top-level try/catch → uncaught exception is `std::terminate` (src/Feed.cpp:258-341, src/SongInstaller.cpp:43-102). Wrap bodies; report failure via `onDone`.
- [x] **Q-M4** — `SanitizeInput` keeps query strings: pasted profile URL with `?tab=...` fails resolution confusingly (src/Feed.cpp:100-110). Strip at `?`/`#`.
- [x] **Q-M5** — Negative curl codes shown as "HTTP -6" (src/SongInstaller.cpp:52, :84; src/Feed.cpp:163). Map `code < 0` to "Network error".
- [x] **Q-M6** — `int args = 2; ... &args` with no-op extract callback (src/SongInstaller.cpp:91-94). Pass `nullptr` or log per-file progress.
- [x] **Q-M7** — README: "no login, no credentials stored" (README.md:18) contradicts the cookie path (:65); dependency list omits SongCore (mod.json requires it).
- [x] **Q-M8** — No `CURLOPT_ACCEPT_ENCODING` → uncompressed 100-score JSON. Set to `""` for gzip.
- [ ] **Q-M9** — Thread-per-image download spike (src/SpriteCache.cpp:43); small shared queue would smooth it. (consciously skipped — see plan coverage map)
...
- [x] **P-C1 — Zip extraction, recursive delete, and all file writes run on the Unity main thread → VR freeze on every map install.**
...
- [x] **P-I1 — Logged-in user with an empty friends feed sees "Couldn't reuse a BeatLeader PC login".**
...
- [x] **P-I2 — BeatLeader installed but not signed in → 8-second stall on effectively every tab visit.**
...
- [x] **P-I3 — An exception in the cookie-auth path aborts the entire fetch, skipping the Unity path and the public fallback.**
...
- [x] **P-I4 — A failed refresh wipes the previously good feed.**
...
- [x] **P-I5 — Failed extraction leaves a partial map folder in CustomLevels.**
...
- [x] **P-I6 — Sprite cache unbounded and never destroys textures.**
...
- [x] **P-I7 — Install completion writes into whichever score's modal is open now, not the one it belongs to.**
...
- [x] **P-I8 — Launch path has no re-entrancy/transition guard and runs outside try/catch.**
...
- [x] **P-M1** — `[OnStart]` async Task is fire-and-forget with unobserved exceptions (pcvr/Plugin.cs:28-34); silent no-tab failure, and `GameplaySetup.Instance == null` at :45 returns silently. Wrap + log; verify BSIPA accepts Task-returning `[OnStart]`.
- [x] **P-M2** — `Escape` renders literal `&amp;`/`&lt;` on screen — TMP doesn't decode entities (pcvr/Utils/Formatting.cs:115). Use `<noparse>` or zero-width-space breaking instead.
- [x] **P-M3** — Hash-case asymmetry in level lookup: tries raw + lowercase, never uppercase; level IDs embed uppercase (pcvr/Services/SongInstaller.cs:31-39). Normalize `ToUpperInvariant()` once.
- [x] **P-M4** — BeatSaver version fallback (`FirstOrDefault()`) can install the wrong map version, then the hash lookup never matches → perpetual "still loading" (pcvr/Services/SongInstaller.cs:52-53). Fail with "map version no longer on BeatSaver" instead.
- [x] **P-M5** — Silent no-op when the open picker doesn't contain the level (pcvr/UI/SnipeFeedView.cs:487-500) — add a status message. Party mode takes the picker branch, diverging from the README table (arguably better; note it).
- [x] **P-M6** — Modal reparent-back relies on HMUI invoking the hide callback for an already-hidden modal (pcvr/UI/SnipeFeedView.cs:523-532, click-off in .bsml:99). If it doesn't, the modal leaks per menu rebuild. Reparent unconditionally in `HideModal`.
- [x] **P-M7** — Scores stepper on the public path only truncates; pull size is config-only `ScoresPerPlayer` (pcvr/Services/BeatLeaderService.cs:94-98) — stepper 100 can never exceed 60. Scale or document.
- [x] **P-M8** — Batch: time-ago/rank strings go stale until rebuild (pcvr/UI/SnipeFeedView.cs:594) *(closed as no-change-needed: OnTabShown rebuilds rows, staleness bounded to a continuously-open tab)*; losing `loginTask` in `Task.WhenAny` left unobserved (BeatLeaderService.cs:320); no decompressed-size ceiling in `ExtractZipSafely` (zip bomb → disk fill) — cap ~2 GB via `entry.Length` sum; raw `ex.Message` interpolated into rich-text fields (SnipeFeedView.cs:265, 412, 439).
